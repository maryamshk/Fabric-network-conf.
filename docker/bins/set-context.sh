...
export ORDERER_ADDRESS=orderer.secureOrderer.com:7050

# RAFT requires TLS
if [ "$2" == "tls" ] || [ "$2" == "raft" ] ; then

    export ORDERER_CA_ROOTFILE=$PWD/config/crypto-config/ordererOrganizations/secureOrderer.com/orderers/orderer.secureOrderer.com/msp/tlscacerts/tlsca.secureOrderer.com-cert.pem
