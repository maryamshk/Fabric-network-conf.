...
# Requires the secureOrderer peer1 to be up & running
#
# Validates the working of : secureOrderer Peer1
#
...
source  set-env.sh  secureOrderer peer1 7050 admin
...
# 6. Execute Query
echo "====> Step 6     Querying A on secureOrderer peer1"
...
# 6. Execute Query
echo "====> Step 8     Querying A on secureOrderer peer1"
peer chaincode query -C $CC_CHANNEL_ID -n $CC_NAME  -c '{"Args":["query","a"]}'
