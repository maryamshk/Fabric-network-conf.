...
echo   '3. Creating the channel: secureOrdererchannel - may fail if already created'
peer channel create -o $ORDERER_ADDRESS -c secureOrdererchannel -f $CONFIG_DIRECTORY/secureOrderer-channel.tx

# If channel already created then -  use the following to fetch channel genesis
#peer channel fetch 0 -o localhost:7050  -c secureOrdererchannel  secureOrdererchannel.block
...
peer channel join -o $ORDERER_ADDRESS -b ./secureOrdererchannel.block
